...
window.startLogin = startLogin; window.logout = logout;
...
// ——————————————————————————————————
// 2. Standings & Upcoming with Sorting & Fallback
// ——————————————————————————————————
async function fetchStandings() {
  ...
  rs.forEach((r,i)=>{
    ...
    rows += `<tr>
      ...
      <td>${r.settings?.waiver_position||i+1}</td>
      <td>—</td>
      <td>—</td>
    </tr>`;
  });
  el.innerHTML = `
    <table>
      <thead>
        <tr>
          <th>Standings</th><th>Team</th><th>W-L</th><th>PF</th><th>PA</th>
          <th>Max PF</th><th>Power Score</th><th>Waiver</th><th>Last</th><th>Next</th>
        </tr>
      </thead>
      <tbody>${rows}</tbody>
    </table>`;
...
async function loadEvents() {
  ...
  const fsEvents = fsSnap.docs.map(d=>({
    ...
    user: d.data().updatedBy||d.data().createdBy||'',
    desc: d.data().desc,
    type: d.data().type
  }));
  // waiver txns
  ...
  const waiverEvents = tx.filter(t=>t.type==='waiver').flatMap(t=>{
    ...
    return Object.keys(t.adds).map(pid=>({
      ts,
      user: userMap[rosterMap[t.adds[pid]]]||'Unknown',
      desc: `📥 ${players[pid]?.full_name||pid}`,
      type: 'Waiver'
    })).concat(Object.keys(t.drops).map(pid=>({
      ts,
      user: userMap[rosterMap[t.drops[pid]]]||'Unknown',
      desc: `❌ ${players[pid]?.full_name||pid}`,
      type: 'Waiver'
    }));
  });
  // merge & sort
  const merged = fsEvents.concat(waiverEvents).sort((a,b)=>b.ts - a.ts).slice(0,20);
  // render table
...
// ——————————————————————————————————
// 4. Teams Page & Analysis Page unchanged…
// Initialization
// ——————————————————————————————————
